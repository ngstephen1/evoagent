...
![Standalone dev accuracy: the fine-tuned 9B STaR Qwen3-8B model (77.91%) beats both frontier APIs — Gemini 2.5-flash (76.20%) and DeepSeek-V3 (66.95%, ~671B) — and the 3-way vote reaches 79.97%.](results_star_vs_api.png)

*Figure 3 — Standalone dev accuracy of each solver, plus the final 3-way ensemble.
Our fine-tuned 9B model (STaR Qwen3-8B) outscores both hosted frontier APIs on its
own — including DeepSeek-V3, which is roughly 70× larger — and the majority vote of
the three adds a further +2.1 points. Generated by
`assignment03/phase3_visualize_results.py`.*
